...
public static class Runner
{
    public static void Run(this IDay day, string path)
    {
        ...
        watch.Restart();
        var part2Result = day.Part2(ReadFile.GetFileStream(path));
        watch.Stop();
...
Console.WriteLine($"| Class Name{"".PadRight(day.GetType().FullName.Length)} | Part | Result{"".PadRight(partColumnWidth)} | Time (ms){"".PadRight(timeColumnWidth)} |");
Console.WriteLine($"|{"".PadRight(day.GetType().FullName.Length + 1, '-')}|------|{"".PadRight(partColumnWidth + 6, '-')}|{"".PadRight(timeColumnWidth + 10, '-')}|");
Console.WriteLine(
